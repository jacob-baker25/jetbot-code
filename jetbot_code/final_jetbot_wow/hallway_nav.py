...
----------
  CRUISE   – PD-centered forward drive; watching for the next turn's opening
             (ignores openings for the first `cruise_delay` seconds)
...
# ── BLE beacon reader (reads from ble_scanner.py's status file) ──────────────
BLE_STATUS_FILE  = "/tmp/jetbot_beacons.json"
BLE_REQUEST_FILE = "/tmp/jetbot_ble_request.json"

def ble_request(target_name):
    """Tell ble_scanner.py which beacon to look for."""
    tmp = BLE_REQUEST_FILE + ".tmp"
    with open(tmp, "w") as f:
        json.dump({"scan": True, "target": target_name}, f)
    os.replace(tmp, BLE_REQUEST_FILE)

def ble_seen(target_name):
    """Return True if ble_scanner.py has ever seen this beacon (no expiry)."""
    try:
        with open(BLE_STATUS_FILE, "r") as f:
            status = json.load(f)
        return bool(status.get(target_name, {}).get("seen"))
...
# ── Route file (written by corner_mapping.py Telegram bot) ───────────────────
ROUTE_FILE        = "/tmp/jetbot_route.json"
CRUISE_DELAY_S    = 5.0   # blind period after each turn before looking for the next

def load_turn_plan():
    """
    Read the route from corner_mapping.py and build TURN_PLAN.
    Each entry: (label, turn_sector, opp_sector, rotation_sign, pivot_fn, cruise_delay_s, beacon_name)
    Falls back to an empty plan (robot just cruises) if no route file exists.
    """
    try:
        with open(ROUTE_FILE, "r") as f:
            route = json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        print(f"[NAV] No route file found at {ROUTE_FILE} — cruising with no turns.")
        return []

    plan = []
    for i, wp in enumerate(route.get("waypoints", []), 1):
        direction = wp.get("direction", "").strip()
        beacon    = wp.get("beacon")
        node      = wp.get("node", "?")

        if direction == "Left":
            entry = (
                f"LEFT  #{i} (node {node})",
                LEFT_SECTOR, RIGHT_SECTOR, -1.0, pivot_left,
                CRUISE_DELAY_S, beacon,
            )
        elif direction == "Right":
            entry = (
                f"RIGHT #{i} (node {node})",
                RIGHT_SECTOR, LEFT_SECTOR, 1.0, pivot_right,
                CRUISE_DELAY_S, beacon,
            )
        else:
            print(f"[NAV] Skipping node {node} — direction '{direction}' needs no turn.")
            continue

        plan.append(entry)

    print(f"[NAV] Loaded route {route.get('start')} → {route.get('goal')}:"
          f" {len(plan)} turn(s).")
    for entry in plan:
        print(f"       {entry[0]}  beacon={entry[6]}")
    return plan

# ── Main ──────────────────────────────────────────────────────────────────────
def run():
    TURN_PLAN = load_turn_plan()
    ...
    if not TURN_PLAN:
        # No route loaded — cruise indefinitely, no turns
        label = turn_sector = opp_sector = rotation_sign = do_pivot = cruise_delay = beacon_name = None
        print("=== Hallway Nav: cruise-only (no route loaded) ===\n")
    else:
        label, turn_sector, opp_sector, rotation_sign, do_pivot, cruise_delay, beacon_name = \
            TURN_PLAN[turn_index]
        if beacon_name:
            ble_request(beacon_name)
        print(f"\n=== Hallway Nav ===")
        print(f"Target rotation per turn: {TARGET_ROTATION_DEG}° ± {ROTATION_TOLERANCE}°\n")
        print(f"[NAV] Cruising — {cruise_delay:.0f}s blind period, then watching"
              f" for turn 1 ({label.strip()})  [beacon gate: {beacon_name}]...")
        ...
        while True:
            ...
            if state == "CRUISE":
                ...
                if int(now * 10) % 2 == 0:
                    ...
                    print(f"[CRUISE] L: {l_str} | R: {r_str} | Steer: {steer:+.3f}")

                # No turns left — just cruise forever
                if turn_index >= len(TURN_PLAN):
                    continue

                cruise_elapsed = now - cruise_start
                looking = cruise_elapsed >= cruise_delay

                if looking:
                    valid, reason = is_real_intersection(scan, turn_sector, opp_sector)
                    open_hits = (open_hits + 1) if valid else 0

                    beacon_ok = (beacon_name is None) or ble_seen(beacon_name)
                    ...
                    if open_hits >= CORNER_OPEN_DEBOUNCE:
                        if beacon_ok:
                            ...
                            open_hits = 0   # reset so we re-debounce after beacon arrives
                            if int(now * 2) % 4 == 0:   # print at ~0.5 Hz
                                print(f"[NAV] Intersection seen but waiting for"
            ...
            # ── TURNING ───────────────────────────────────────────────
                ...
                if done_hits >= ROTATION_DONE_DEBOUNCE or elapsed > TURN_TIMEOUT_S:
                    ...
                    if turn_index >= len(TURN_PLAN):
                        state = "CRUISE"   # keep going — no more turns
                        cruise_start = now
                        label = beacon_name = None
                        print("\n[NAV] All turns complete. Continuing to cruise.")
                    else:
                        label, turn_sector, opp_sector, rotation_sign, do_pivot, cruise_delay, beacon_name = \
                            TURN_PLAN[turn_index]
                        open_hits    = 0
                        ...
                        if beacon_name:
                            ble_request(beacon_name)
                        print(f"[NAV] Cruising — {cruise_delay:.0f}s blind period, then"
                              f" watching for turn {turn_index + 1} ({label.strip()})"
                              f"  [beacon gate: {beacon_name}]...")
